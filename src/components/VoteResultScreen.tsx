...
interface VoteResultScreenProps {
  ...
  onProceedNextRound: () => void | boolean | Promise<boolean | void>;
  onProceedToTruth: (winner: 'innocents' | 'guilty') => void | boolean | Promise<boolean | void>;
  onBack?: () => void;
  onNavigateHome?: () => void;
  language?: 'ar' | 'en';
...
export const VoteResultScreen: React.FC<VoteResultScreenProps> = ({
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center bg-[#07080c] select-none text-slate-100 pb-16 pt-4 px-3 sm:px-6" dir={isRtl ? 'rtl' : 'ltr'}>
          ...
          {isGameOver ? (
            <motion.button
              ...
              onClick={async () => {
                ...
                sound.playClick();
                try {
                  const res = await onProceedToTruth(winner);
                  if (res === false) {
            ...
            <motion.button
              ...
              onClick={async () => {
                ...
                sound.playClick();
                try {
                  const res = await onProceedNextRound();
                  if (res === false) {
                    setIsProceeding(false);
                  }
                } catch {
      ...
      {/* Confirmation Modal when Leaving Active Game Session */}
      <AnimatePresence>
        {showExitConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            ...
            >
              <h3 className="text-lg font-black text-[#f3cb79]">
                {isEn ? 'Leave Active Game?' : 'مغادرة التحقيق الحالي؟'}
              </h3>
              <p className="text-xs sm:text-sm text-[#b0a99c] leading-relaxed">
                {isEn
                  ? 'Are you sure you want to abandon the current case and return to the main menu?'
                  : 'هل أنت متأكد من رغبتك في مغادرة التحقيق الحالي والعودة إلى القائمة الرئيسية؟'}
              </p>
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isEn ? 'Stay in Game' : 'البقاء في اللعبة'}
                </button>
                <button
                  onClick={() => {
