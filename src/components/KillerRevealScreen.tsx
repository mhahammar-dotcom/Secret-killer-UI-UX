    ...
    <div className="relative min-h-screen w-full flex flex-col items-center bg-[#07080c] select-none text-slate-100 pb-16 pt-4 px-3 sm:px-6" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="relative z-10 w-full max-w-xl flex flex-col justify-between flex-1 gap-5">
        ...
        <div className="flex items-center justify-between w-full pt-2 pb-1 border-b border-amber-900/20">
          ...
          <div className="text-center">
            <h1 className={`text-2xl font-black ${isRtl ? "font-['Cairo']" : 'font-sans'} text-[#f5ebd9] tracking-wide leading-tight drop-shadow-md`}>
              {guiltyPlayers.length > 1 ? t.theCulpritsWere : t.killerReveal}
            </h1>
        ...
        <div className="my-auto flex flex-col items-center w-full">
          {guiltyPlayers.length <= 1 ? (
            /* Single Killer Layout */
            <motion.div
            ...
            /* Multi-Killer Layout (2 or 3 Killers) */
            <div className="flex flex-col items-center w-full gap-4">
              <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-red-950/40 border border-red-500/50 text-red-300 text-xs font-black">
                <Skull className="w-4 h-4 text-red-400" />
                <span>
                  {isEn ? `${guiltyPlayers.length} Perpetrators in Alliance` : `تحالف من ${guiltyPlayers.length} جناة`}
                </span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 w-full">
                {guiltyPlayers.map((killer) => {
                  const otherPartners = guiltyPlayers.filter((p) => p.id !== killer.id);
                  return (
                    <motion.div
                      key={killer.id}
                      initial={{ opacity: 0, y: 15 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="relative rounded-2xl bg-[#0d0f16] border-2 border-red-500/40 p-4 shadow-xl flex flex-col items-center text-center overflow-hidden"
                    >
                      <div className="relative w-28 h-28 rounded-xl overflow-hidden border border-red-500/60 mb-2.5 shadow-md">
                        <img
                          src={culpritRevealImg}
                          alt={t.killerRole}
                          className="w-full h-full object-cover grayscale contrast-125 brightness-90"
                        ...
                        />
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <span className="stamp-killer text-xs font-black tracking-widest uppercase">
                            {t.killerRole}
                          </span>
                        </div>
                      </div>

                      <h4 className={`text-lg font-black text-red-400 ${isRtl ? "font-['Cairo']" : 'font-sans'}`}>
                        {killer.character.name}
                      </h4>
                      <span className={`text-xs text-[#d4cfc7] font-bold ${isRtl ? "font-['Cairo']" : 'font-sans'}`}>
                        ({killer.name})
                      </span>
                      <span className={`text-[11px] text-[#a39a8c] ${isRtl ? "font-['Cairo']" : 'font-sans'} mt-0.5`}>
                        {killer.character.profession}
                      </span>

                      {/* Partners */}
                      {otherPartners.length > 0 && (
                        <div className={`mt-2 pt-2 border-t border-red-900/30 w-full text-[11px] text-red-300/90 font-medium ${isRtl ? "font-['Cairo']" : 'font-sans'}`}>
                          <span className="text-[#a39a8c] block text-[10px]">
                            {isEn ? 'Partners in crime:' : 'الشركاء في الجريمة:'}
                          </span>
                          <span>
                            {otherPartners.map((p) => `${p.character.name} (${p.name})`).join(', ')}
                          </span>
                        </div>
                      )}
                    </motion.div>
              ...
              {/* Confession Box */}
              <div className={`w-full px-4 py-3 rounded-2xl bg-[#141724] border border-[#c8923a]/40 text-[#f5ebd9] text-xs sm:text-sm font-bold text-center ${isRtl ? "font-['Cairo']" : 'font-sans'} leading-relaxed`}>
                {getConfession()}
              </div>
            </div>
          )}
